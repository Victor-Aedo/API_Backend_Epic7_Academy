...
async def character_list_image():
    ...
    convert_images_to_base85(directory_path) # Llamar a la función para obtener y convertir las imágenes a base85
    # for i in test:
    #     print('name: ', i['name'], 'image: ', i['image'])
            ...
            for  objeto in characters:
                if (objeto['name'] == os.path.splitext(filename)[0]):
                    ...
                    with open(file_path, 'rb') as f:
                        image_data = f.read()
                        test.append({'name': os.path.splitext(filename)[0], 'image': image_data}) 
